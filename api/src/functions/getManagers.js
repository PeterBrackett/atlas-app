...
// Same Graph/SharePoint pattern as getData.js and getInstitutions.js, so
// manager files can be edited in SharePoint like every other Atlas data
// product, with a same-shaped GitHub-hosted static file as fallback.
...
app.http('getManagers', {
  methods: ['GET'],
  authLevel: 'anonymous',
  route: 'managers/{country}',
  handler: async (request, context) => {
    ...
    const fileName = `${country}_managers.json`;
    ...
    try {
      ...
      if (graphResponse.status === 404) {
        return { status: 404, jsonBody: { error: `No manager data file found for '${country}'` } };
      }
